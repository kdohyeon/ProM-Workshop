 ...
 * - Change resource:
 * - Change all:
 * - Add a new event (new activity, time, resource):
 * - Remove an event
 * */
...
public class MutatorModel {
	...
	public MutatorModel(XLog log, MutatorParameters parameters) throws IOException, ParseException {
		...
		for(int a = 0; a < types.length; a++) {
			for(int b = 0; b < dps.length; b++) {
				String thisType = types[a];
				float thisDp = dps[b];
				...
				outputPath = "C:\\Users\\dhkim-pc\\Desktop\\Evaluation_PLG\\";
				...
				for(int i = 0; i < caseSize; i++) {
					...
					if(randomFloat.getRandomNumberElement(i) <= dp) {
						...
						}else if(type.equals("Add")) { // if deviation type == "Add"
							String resource = "";
						...
						}else if(type.equals("Remove")) { // else if deviation type == "Remove"
						...
						}else if(type.equals("Replace")) { // else if deviation type == "Replace"
	...
	private String getRandomResource() {
		String[] newResourceName = {
				...
				"Resource R4",
				"Resource R5"};
		RandomInteger randomRes = new RandomInteger(1, 0, newResourceName.length-1);
	...
	private String getRandomActivity() {
		...
		RandomInteger randomAct = new RandomInteger(1, 0, newActivityName.length-1);

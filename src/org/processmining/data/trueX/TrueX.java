package org.processmining.data.trueX;
...
public class TrueX {
	...
	public TrueX(String fromActivityID, String toActivityID, String fromResourceID, String toResourceID, String relation) {
	...
	public float calculateSum() {
		...
		for(int i = 0; i < relationList.size(); i++) {
			result = result + relationList.get(i).getTrueXTime();
		}
	...
	public float calculateStdev() {
		...
		for(int i = 0; i < relationList.size(); i++){
			result = (float) (result + Math.pow((relationList.get(i).getTrueXTime() - avg), 2));
		}
	...
	public float calculateMin() {
		...
		result = relationList.get(0).getTrueXTime();
		for(int i = 1; i < relationList.size(); i++)
		{
			if(result > relationList.get(i).getTrueXTime())
			{
				result = relationList.get(i).getTrueXTime();
			}
		}

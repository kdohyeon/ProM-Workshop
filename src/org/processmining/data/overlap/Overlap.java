package org.processmining.data.overlap;
...
public class Overlap {
	...
	public Overlap(String fromActivityID, String toActivityID, String fromResourceID, String toResourceID, String relation) {
	...
	public float calculateSum() {
		...
		for(int i = 0; i < relationList.size(); i++) {
			result = result + relationList.get(i).getOverlapTime();
		}
	...
	public float calculateStdev() {
		...
		for(int i = 0; i < relationList.size(); i++){
			result = (float) (result + Math.pow((relationList.get(i).getOverlapTime() - avg), 2));
		}
	...
	public float calculateMin() {
		...
		result = relationList.get(0).getOverlapTime();
		for(int i = 1; i < relationList.size(); i++)
		{
			if(result > relationList.get(i).getOverlapTime())
			{
				result = relationList.get(i).getOverlapTime();
			}
		}
